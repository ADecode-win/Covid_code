document.addEventListener("DOMContentLoaded", function() {
    ...
    let circlesVisible = false;  // NEW: State to track visibility of circles

    const toggleButton = d3.select("#toggle-btn");  // NEW: Select the toggle button
    toggleButton.on("click", function() {  // NEW: Add click event listener
            circlesVisible = !circlesVisible;  // NEW: Toggle the visibility state
            svg.selectAll("circle").style("display", circlesVisible ? null : "none");  // NEW: Apply visibility
            this.textContent = circlesVisible ? "Hide Data Points" : "Show Data Points";  // NEW: Update button text
        });
    ...
    d3.json("filtered_european_data.json").then(function(data) {
      ...
      const updateChart = function(date) {
        ...
        svg.append("path")
            ...
            svg.selectAll("circle")  // NEW: Modified to create circles conditionally based on circlesVisible
            .data(filteredData)
            .enter()
            .append("circle")
            .attr("cx", d => x(d.date))
            ...
            .attr("r", 5)
            .attr("fill", "red")
            .style("display", circlesVisible ? null : "none")  // NEW: Apply initial visibility state
